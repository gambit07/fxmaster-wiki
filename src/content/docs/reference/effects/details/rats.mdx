---
title: "Rats"
description: "Ground-level rat swarms with token avoidance and directional movement controls."
sidebar:
  order: 29
  badge:
    text: "FXMaster"
...
<div class="fxm-badge-row">
  ...
  <span class="fxm-badge">API type: rats</span>
</div>

<EffectShowcase src="/videos/rats.mp4" title="Rats" description="Preview shows Rats in Directional Movement mode with Directional Spread enabled" />

Ground-level rat swarms with token avoidance and directional movement controls.
...
- **Directional Movement:** Makes rats travel in the configured Direction instead of relying only on the effect's default random movement.
- **Token Avoidance:** Makes rats steer away from eligible tokens instead of moving directly through them.
- **Avoid Stationary Tokens:** Keeps rats avoiding eligible tokens even when those tokens are not moving. Disable it when only token movement should cause a reaction.
- **Avoidance Radius:** Sets how far from an eligible token rats begin changing course.
- **Avoidance Strength:** Controls how sharply and forcefully rats turn away from eligible tokens.
